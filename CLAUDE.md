...
- **Internal links end with `/`** (`/posts/<slug>/`, not `/posts/<slug>`): Cloudflare Pages 308-redirects slash-less URLs, and `trailingSlash: "always"` makes dev/preview 404 on them. `getPath()` link URLs already include the slash.
