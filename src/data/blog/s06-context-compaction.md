...
The compression strategy works in layers, each more aggressive than the last.

Layer 1 — **micro-compact** — runs silently before every API call. It scans the messages array for old tool results (anything beyond the three most recent) and replaces their content with a short placeholder like `"[Previous: used read_file]"`. The model still sees that a tool was called and what kind. The actual output — the 500-line file, the verbose bash dump — is dropped. That's the whole point: old tool results have usually done their job by the time micro-compact catches them, the recent three are kept untouched as the active working set, and anything older can be re-fetched by calling the tool again if needed. No API call, no summarization, just structural pruning — the cheapest of the three layers.
