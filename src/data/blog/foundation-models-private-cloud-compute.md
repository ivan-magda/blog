...
At WWDC26, Apple shipped a server model with none of that. `PrivateCloudComputeLanguageModel` runs on the same Private Cloud Compute infrastructure behind Apple Intelligence and plugs into the Foundation Models framework as a one-line change — no account setup, no authentication, no API keys, and no cloud API cost to us as developers. The trade: it needs iOS 27, an entitlement, and each user gets a daily request limit.
...
When a user over their limit calls `respond`, the framework throws specifically `PrivateCloudComputeLanguageModel.Error.quotaLimitReached`, so the path of least resistance is a `catch` block that pops an alert. Apple's session argues, and I agree, that this is the wrong pattern. An alert implies something went wrong and disappears when dismissed, but the quota is neither wrong nor temporary — it stays exhausted until it resets. The user dismisses the alert, taps the button again, and gets the same alert. Nothing in the UI tells them the feature is unavailable _before_ they try.

The framework gives us what we need to do better: `model.quotaUsage`, a queryable state we can drive persistent UI from, before any request is made. There are three pieces: a `status` that reports `.belowLimit` with an `isApproachingLimit` flag, an `isLimitReached` Bool, and a `limitIncreaseSuggestion` that can present the upgrade path. Here's the footer of our summarizer view, handling all three:
...
Next, the OS floor. `PrivateCloudComputeLanguageModel` ships in iOS 27, macOS 27, watchOS 27, and visionOS 27, so the real availability check starts with `#available(iOS 27.0, *)` and falls back to `SystemLanguageModel` on older systems.
